...
LDDE * diffLists(LDDE *l1, LDDE *l2)
{
    LDDE *d = (LDDE *) malloc(sizeof(LDDE));
    initList(d);
    ...
    while (atualL1 != NULL && atualL2 != NULL)
    {
        ...
        {
            incluir(d, atualL1->valor);
            atualL1 = atualL1->proximo;
        }
        else if (atualL1->valor > atualL2->valor)
        {
...
int main()
{
    ...
    LDDE *listaDAB = diffLists(&l1, &l2);
    LDDE *listaDBA = diffLists(&l2, &l1);

    printHeadToTail(listaDAB);
    printTailToHead(listaDAB);
    printHeadToTail(listaDBA);
    printTailToHead(listaDBA);
    return 0;
}