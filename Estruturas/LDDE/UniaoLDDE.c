...
LDDE * unionLists(LDDE *l1, LDDE *l2)
{
    LDDE *u = (LDDE *) malloc(sizeof(LDDE));
    initList(u);
    ...
    while (atualL1 != NULL && atualL2 != NULL)
    {
        ...
        {
            incluir(u, atualL1->valor);
            atualL1 = atualL1->proximo;
        }
        else if (atualL2->valor < atualL1->valor)
        {
            incluir(u, atualL2->valor);
...
int main()
{
    ...
    LDDE *uniao = unionLists(&l1, &l2);

    printHeadToTail(uniao);
    printTailToHead(uniao);
